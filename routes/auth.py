...
@USERS.post("/register" ,response_model=schemas.UserRead)
def create_user(user_details:schemas.UserCreate,db=Depends(get_db)):
    password=auth.password_hash(user_details.password)
...
@USERS.post("/update",response_model=schemas.Update_Password_Message)
def update_info(user_data:schemas.Update_Password,user_id:int=Depends(oath2.get_current_user),db=Depends(get_db)):
    user=db.query(models.User).filter(user_id == models.User.id).first()
    user.password=auth.password_hash(user_data.password)
    db.commit()
    db.refresh(user)
    return {"password":user.password,"message":"password Updated Succesfully "}

    
@USERS.post("/admin/roles/{user_id}",response_model=schemas.UserRead)
